...
class SantaRunGame {
    ...
    init() {
        this.container.innerHTML = `
            <div class="minigame-container">
                <div class="game-header">
                    <div class="game-score">🎅 Meter: <span id="run-distance">0</span>m</div>
                    <div class="game-score">🎁 Geschenke: <span id="run-gifts">0</span></div>
                </div>
                <div class="run-canvas" id="run-canvas">
                    ...
                    <div class="run-instructions">
                        ...
                        <p>⬅️ ➡️ Pfeiltasten oder Wischen zum Bewegen</p>
                        <p>🎁 Sammle Geschenke und weiche Hindernissen aus!</p>
    ...
    startSpawning() {
        const spawn = () => {
            ...
            // Garantiere mindestens 1 Objekt, 30% Chance für 2 Items
            const spawnTwo = Math.random() < 0.3;
            const usedLanes = [];
            
            if (spawnTwo) {
                // Bei 2 Items: Mindestens 1 muss ein Geschenk sein!
                // Wähle 2 verschiedene Bahnen
                ...
                const bothPresents = Math.random() < 0.3; // 30% Chance für 2 Geschenke
                ...
                if (bothPresents) {
                    ...
                    // 1 Hindernis + 1 Geschenk (Reihenfolge zufällig)
                    if (Math.random() < 0.5) {
            ...
            // Dynamisches Spawn-Intervall - viel kürzere Pausen!
    ...
    update() {
        ...
        // Alle 20 Meter wird es schneller und häufiger
    ...
    endGame() {
        ...
        overlay.className = 'game-over';
        overlay.innerHTML = `
            <div class="game-over-title">🎅 Santa ist müde! 🎄</div>
            <div class="game-over-score">Du bist <strong>${Math.floor(this.distance)}</strong> Meter gelaufen!</div>
            <div class="game-over-score">Geschenke gesammelt: <strong>${this.gifts}</strong> 🎁</div>
            <div class="game-over-message">${this.getScoreMessage()}</div>
        `;
        ...
        document.getElementById('run-start-button').textContent = 'Nochmal laufen! 🔄';
    }
    
    getScoreMessage() {
        if (this.distance >= 200) return '🌟 Unglaublich! Du bist ein Marathon-Santa!';
        if (this.distance >= 150) return '⭐ Fantastisch! Tolle Ausdauer!';
        if (this.distance >= 100) return '✨ Super! Sehr gute Leistung!';
        if (this.distance >= 50) return '🎄 Gut gemacht! Weiter so!';
        return '🎅 Nicht schlecht! Versuch es nochmal!';
    }
}
